...
export const verifyUSDTTransfer = (
  ...
  if (
    !transferDetails.tokenTransfers ||
    transferDetails.tokenTransfers.length === 0
  ) {
    return {
      isValid: false,
      message: "No token transfer events found in the transaction details.",
    };
  }
  ...
  if (
    ...
    return {
      isValid: false,
      message: "Recipient address does not match the company's wallet address.",
    };
  }
  ...
  if (
    ...
    return {
      isValid: false,
      message: "Token mint address does not match the expected USDT address.",
    };
  }
  ...
  if (expectedDescription !== transferDetails.description) {
    return {
      isValid: false,
      message: "Transaction description does not match the expected format.",
    };
  }
